<?php
	...
	$sqlra2 =  "SELECT * FROM `gcb_art` WHERE `refart`='$rowb[refart]' LIMIT 1 ";
	...
	if(!$qra2){ print("* ".mysqli_error($db)."</br>");
		...
		while($rowaut2 = mysqli_fetch_assoc($qra2)){
			...
			global $email;
			$email = "<a style=\"display: inline-block;\" href=\"Gcb.Www/portfolio.php?portfolio=".$rowb['refart']."#formlink\">FORMULARIO RESERVAS</a>";
			global $tlf;
	...
	global $veropina;
	$veropina = "<form style=\"display: inline-block;\" name='veropina' action='Gcb.Syst/Opina_Cliente_Ver.php' target='popup' method='POST' onsubmit=\"window.open('', 'popup', 'width=420px, height=580px')\">
	...
	global $opina;
	$opina = "<form style=\"display: inline-block;\" name='creaopina' action='Gcb.Syst/Opina_Cliente.php' target='popup' method='POST' onsubmit=\"window.open('', 'popup', 'width=420px, height=580px')\">
	...
	<img src='".$rut."Gcb.Img.Art/".@$_POST['myimg1']."' style=\" width:98%; max-width:700px; height:auto\" />
	".$veropina.$opina."<form style=\"display: inline-block;\" name='ver' name='ver' action=\"index.php#".$rowb['refart']."\" method='post' >
