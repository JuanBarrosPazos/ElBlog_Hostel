...
function process_form(){
	...
	$destination_file = '../Gcb.Img.Admin/'.$safe_filename;
	

	if( file_exists( '../Gcb.Img.Admin/'.$safe_filename) ){
		unlink('../Gcb.Img.Admin/'.$safe_filename);
			}
			
	elseif (move_uploaded_file($_FILES['myimg']['tmp_name'], $destination_file)){
		
		if( file_exists( '../Gcb.Img.Admin/'.$_SESSION['smyimg'])){
			unlink('../Gcb.Img.Admin/'.$_SESSION['smyimg']);
		}else{}
		...
		$rename_filename = "../Gcb.Img.Admin/".$new_name;	
	...
	<img src='../Gcb.Img.Admin/".$_SESSION['new_name']."' height='120px' width='90px' />
					</td>
				</tr>
...
function show_form($errors=''){
	...
	print("<table align='center'  border=0 style='margin-top:20px; width:95.5%'>
				...
				LA IMAGEN ACTUAL DE : </br>".$defaults['Nombre']." ".$defaults['Apellidos'].".
					...
					<th class='BorderInf'>
<img src='../Gcb.Img.Admin/".$_SESSION['smyimg']."' height='120px' width='90px' />
					</th>
				</tr>
