...
if (preg_match('/^(\w{1})*(\s\w{1})/',$_POST['Apellidos'],$ref4)){	$rf4 = $ref4[2];
	...
	/* COMPROBAMOS SI EXISTE EL ADMINISTRADOR */
		...
		$admin =  "SELECT * FROM `$db_name`.`gch_admin` WHERE `ref` = '$rf'";
...
////////////////////				////////////////////				////////////////////
	...
	$sqldni =  "SELECT * FROM `$db_name`.`gch_admin` WHERE `gch_admin`.`dni` = '$_POST[dni]'";
...
////////////////////				////////////////////				////////////////////
	...
	$sqlml =  "SELECT * FROM `$db_name`.`gch_admin` WHERE `gch_admin`.`Email` = '$_POST[Email]'";
...
////////////////////				////////////////////				////////////////////
	...
	$sqlus =  "SELECT * FROM `$db_name`.`gch_admin` WHERE `gch_admin`.`Usuario` = '$_POST[Usuario]'";
...
////////////////////				////////////////////				////////////////////
	...
	$sqltlf1 =  "SELECT * FROM `$db_name`.`gch_admin` WHERE `gch_admin`.`Tlf1` = '$_POST[Tlf1]' OR `gch_admin`.`Tlf2` = '$_POST[Tlf1]' ";
...
////////////////////				////////////////////				////////////////////
	...
	if(strlen(trim($_POST['Tlf2'])) > 0){
			...
			$sqltlf2 =  "SELECT * FROM `$db_name`.`gch_admin` WHERE `gch_admin`.`Tlf1` = '$_POST[Tlf2]' OR `gch_admin`.`Tlf2` = '$_POST[Tlf2]'";
