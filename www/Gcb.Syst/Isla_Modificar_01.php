...
function isla_borra(){
	...
	print("<table align='center' style=\"margin-top:10px\">
		...
		if(!$qb){ print("<font color='#FF0000'>Consulte L.94: </font></br>".mysqli_error($db)."</br>");
									...
									unlink("../Gcb.Img.Art/".$rowdi['myimg']);
										}
									}
								}

	// BORRO LOS ARTICULOS
	$sqld2 = "DELETE FROM `$db_name`.`gcb_art` WHERE `gcb_art`.`refisla` = '$refisla' ";
	if(mysqli_query($db, $sqld2)){}
	else {print("NO SE HAN ACTUALIZADO LAS ENTRADAS EN gcb_art ".$refisla."* MODIFIQUE LA ENTRADA L.207: ".mysqli_error($db));}

	// BORRO LOS ayuntamientos
	$sqld3 = "DELETE FROM `$db_name`.`gcb_aytos` WHERE `gcb_aytos`.`refisla` = '$refisla' ";
	if(mysqli_query($db, $sqld3)){}
	else {print("NO SE HAN ACTUALIZADO LAS ENTRADAS EN gcb_aytos ".$refisla."* MODIFIQUE LA ENTRADA L.207: ".mysqli_error($db));}
